...
- [用干净 IP 访问 Claude：VPS + VPN + Clash 完整配置教程](#用干净-ip-访问-claudevps--vpn--clash-完整配置教程)
  - [适用人群](#适用人群)
  - [目录](#目录)
  - [整体思路](#整体思路)
  - [第一步：购买 VPS 服务器](#第一步购买-vps-服务器)
    - [VMiss 购买步骤](#vmiss-购买步骤)
    - [✅ 先查 IP 质量](#-先查-ip-质量)
  - [第二步：用 SSH 连接 VPS 服务器](#第二步用-ssh-连接-vps-服务器)
    - [VMiss：密码登录（推荐）](#vmiss密码登录推荐)
    - [DMIT：PEM 私钥登录](#dmitpem-私钥登录)
  - [第三步：在 VPS 上安装 VPN 服务](#第三步在-vps-上安装-vpn-服务)
    - [1. 安装基础环境](#1-安装基础环境)
    - [2. 安装 v2ray-agent](#2-安装-v2ray-agent)
    - [3. 选择协议](#3-选择协议)
    - [4. 获取订阅链接](#4-获取订阅链接)
  - [第四步：安装 Clash 并导入订阅](#第四步安装-clash-并导入订阅)
    - [1. 安装 Clash Verge Rev](#1-安装-clash-verge-rev)
    - [2. 导入订阅链接](#2-导入订阅链接)
    - [3. 网络设置](#3-网络设置)
  - [第五步：DNS 防泄露（简要）](#第五步dns-防泄露简要)
    - [1. DNS 定向优化（可选）](#1-dns-定向优化可选)
    - [2. 浏览器防泄露（推荐）](#2-浏览器防泄露推荐)
    - [3. DNS 泄露怎么测（简要）](#3-dns-泄露怎么测简要)
    - [4. 防 DNS 泄露的排查思路（不展开）](#4-防-dns-泄露的排查思路不展开)
  - [第六步：验证配置是否成功](#第六步验证配置是否成功)
    - [基础验证](#基础验证)
    - [进阶验证（可选）](#进阶验证可选)
  - [常见问题排查](#常见问题排查)
    - [服务器突然断连 / 无法访问](#服务器突然断连--无法访问)
  - [推广链接](#推广链接)
  - [致谢](#致谢)
...
> 如果已经开启 TUN，通常更建议关闭**系统代理**，避免双重接管导致行为复杂、排障困难。
> **IPv6** 建议先关闭，优先减少 IPv6 直连或 IPv6 DNS 造成的泄露路径。
...
如果你的目标不只是“能上网”，而是“浏览器和 AI CLI 工具都尽量别泄露真实 DNS”，建议在配置完成后顺手测一下。

**浏览器怎么测**

- 访问 [dnsleaktest.com](https://www.dnsleaktest.com/)，点 Standard test
- 如果测出来的 DNS 服务器还是你本地运营商，说明仍有 DNS 泄露风险
- 如果显示的是代理出口所在地区的解析器，通常说明浏览器侧更干净
  ![测试DNS是否泄漏.png](images/测试DNS是否泄漏.png)

**CLI 怎么测**

- 重点不要只测浏览器，也要单独测 `Claude Code`、`Codex` 这类工具
- 一个实用思路是：让 CLI 发起一个真实请求，同时看它有没有先连到 Clash 本地代理端口

例如：

```bash
claude -p "Please think for 10 seconds, then reply with exactly: OK"
lsof -nP -i | rg 'claude|7897'
```

如果能看到类似 `127.0.0.1:* -> 127.0.0.1:7897` 的连接，通常说明 CLI 是先把请求交给 Clash，而不是自己直接出网。

### 4. 防 DNS 泄露的排查思路（不展开）

这里不展开具体参数，只给一个适合新手的思路：

1. 先分开看“浏览器”和“CLI”，不要只测其中一个
2. 如果浏览器泄露，优先检查 Clash 是否真的在接管 DNS
3. 如果 CLI 泄露，重点看它是不是走了 Clash 本地代理端口，或者是不是直接用了系统 DNS
4. 建议用 **VS Code + Codex 插件** 打开 Clash 配置目录，定位当前真正生效的是哪份配置文件，再逐步排查
5. 不要一上来就改你眼前那份 `config.yaml`，要先确认当前实际加载的是不是它

重点不是“背一堆 DNS 参数”，而是先确认：

- 浏览器的 DNS 有没有泄露
- CLI 的 DNS 有没有泄露
- 你改的是不是当前真正生效的 Clash 配置
